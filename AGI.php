 ...
 *
 *             $Id: AGI.php,v 2.20 2010/09/30 02:21:00 masham Exp $
 *
...
class AGI
{
     ...
     * @var int
     */
    public static $timeoutSecondsInput = 120;

    /**
     * 
     * @var int
     */
    public static $timeoutSecondsOutput = 120;
    ...
    const AUDIO_FILENO = 3;

    /**
     * Constructor
     *
     * @param string $config
     *            is the name of the config file to parse
     * @param array $optconfig
     *            is an array of configuration vars and vals, stuffed into $this->config['phpagi']
     */
    public function __construct($config = NULL, $optconfig = array())
    {
        ...
        elseif (file_exists(static::DEFAULT_PHPAGI_CONFIG))
            $this->config = parse_ini_file(static::DEFAULT_PHPAGI_CONFIG, true);
        ...
        if (!isset($this->config['phpagi']['tempdir']))
            $this->config['phpagi']['tempdir'] = static::AST_TMP_DIR;
        ...
        $this->setSocketTimeoutInput(static::$timeoutSecondsInput);
        $this->setSocketTimeoutOutput(static::$timeoutSecondsOutput);

        $this->conlog('PHPAGI Started');
        $this->conlog('- Socket read timeout set to ' . static::$timeoutSecondsInput . ' seconds');
        $this->conlog('- Socket write timeout set to ' . static::$timeoutSecondsOutput . ' seconds');

        // initialize error handler
        if ($this->config['phpagi']['error_handler'] == true) {
            set_error_handler([$this, 'phpagiErrorHandler']);
        ...
        $str = fgets($this->in);
        while ($str != "\n") {
            $this->request[substr($str, 0, strpos($str, ':'))] = trim(
    ...
    public function channel_status($channel = '')
    {
        ...
        switch ($ret['result']) {
            case -1:
                ...
                break;
            case static::AST_STATE_DOWN:
                $ret['data'] = 'Channel is down and available';
                break;
            case static::AST_STATE_RESERVED:
                $ret['data'] = 'Channel is down, but reserved';
                break;
            case static::AST_STATE_OFFHOOK:
                $ret['data'] = 'Channel is off hook';
                break;
            case static::AST_STATE_DIALING:
                $ret['data'] = 'Digits (or equivalent) have been dialed';
                break;
            case static::AST_STATE_RING:
                $ret['data'] = 'Line is ringing';
                break;
            case static::AST_STATE_RINGING:
                $ret['data'] = 'Remote end is ringing';
                break;
            case static::AST_STATE_UP:
                $ret['data'] = 'Line is up';
                break;
            case static::AST_STATE_BUSY:
                $ret['data'] = 'Line is busy';
                break;
            case static::AST_STATE_DIALING_OFFHOOK:
                $ret['data'] = 'Digits (or equivalent) have been dialed while offhook';
                break;
            case static::AST_STATE_PRERING:
     ...
     *            file to play. Do not include file extension.
     ...
     * Waits up to $timeout milliseconds for
     ...
     * time has passed.
     * Optionally the file BEEP is played before recording begins.
     *
     * @link http://www.voip-info.org/wiki-record+file
     * @param string $file
     *            to record, without extension, often created in /var/lib/asterisk/sounds
     * @param string $format
     *            of the file. GSM and WAV are commonly used formats. MP3 is read-only and thus cannot be used.
     * @param string $escape_digits
     * @param integer $timeout
     *            is the maximum record time in milliseconds, or -1 for no timeout.
     * @param integer $offset
     *            to seek to without exceeding the end of the file.
     * @param boolean $beep
     * @param integer $silence
     *            number of seconds of silence allowed before the function returns despite the
     ...
     */
    public function say_alpha($text, $escape_digits = '')
    {
     ...
     * @param integer $time
     *            number of seconds elapsed since 00:00:00 on January 1, 1970, Coordinated Universal Time (UTC).
     * @param string $escape_digits
     * @return array, see evaluate for return information. ['result'] is -1 on hangup or error, 0 if playback completes with no
     *         digit received, otherwise a decimal value of the DTMF tone. Use chr() to convert to ASCII.
     */
     ...
     * @param string $image
     *            without extension, often in /var/lib/asterisk/images
     ...
     * The variables so created can later be used by later using ${<variablename>}
     * in the dialplan.
     *
     ...
     * This command is similar to the GET DATA
     ...
     * @param string $filename
     *            without extension, often in /var/lib/asterisk/sounds
     * @param string $escape_digits
     * @param integer $offset
     * @return array, see evaluate for return information. ['result'] is -1 on hangup or error, 0 if playback completes with no
     *         digit received, otherwise a decimal value of the DTMF tone. Use chr() to convert to ASCII.
     ...
     * @param integer $timeout
     *            in millisecons. Use -1 for the timeout value if you want the call to wait indefinitely.
    ...
    public function exec_dial(
        ...
        return $this->exec(
            'Dial',
            trim(
                "$type/$identifier" . $this->option_delim . $timeout . $this->option_delim .
                    $options . $this->option_delim . $url,
                $this->option_delim
            )
        );
    }
    ...
    public function exec_goto($a, $b = NULL, $c = NULL)
    {
        return $this->exec(
            'Goto',
            trim(
                $a . $this->option_delim . $b . $this->option_delim . $c,
                $this->option_delim
            )
        );
    }
    ...
    public function fastpass_say_digits(&$buffer, $digits, $escape_digits = '')
    {
        ...
        if ($buffer == '' || $proceed) {
            ...
            if ($res['code'] == static::AGIRES_OK && $res['result'] > 0)
                $buffer .= chr($res['result']);
            return $res;
        }
        return array(
            'code' => static::AGIRES_OK,
     ...
     *         digit received, otherwise a decimal value of the DTMF tone. Use chr() to convert to ASCII.
     */
    public function fastpass_stream_file(
        &$buffer,
        $filename,
        $escape_digits = '',
        $offset = 0
     ...
     * @return array, see evaluate for return information.
     */
    public function fastpass_text2wav(
        &$buffer,
        $text,
        $escape_digits = '',
        $frequency = 8000
     ...
     * @return array, see evaluate for return information.
     */
    public function fastpass_swift(
        &$buffer,
        $text,
        $escape_digits = '',
        $frequency = 8000,
     ...
     * @return array, see evaluate for return information.
     */
    public function fastpass_say_punctuation(
        &$buffer,
        $text,
        $escape_digits = '',
        $frequency = 8000
     ...
     *         This differs from other commands with return DTMF as numbers representing ASCII characters.
     */
    public function fastpass_get_data(
        &$buffer,
        $filename,
        $timeout = NULL,
        $max_digits = NULL
    ...
    public function menu($choices, $timeout = 2000)
    {
        ...
        while (is_null($choice)) {
            foreach ($choices as $prompt) {
                ...
                if ($ret['code'] != static::AGIRES_OK || $ret['result'] == -1) {
                    $choice = -1;
                    break;
                }
    ...
    public function text2wav($text, $escape_digits = '', $frequency = 8000)
    {
        ...
        if (!file_exists("$fname.wav")) {
            ...
            shell_exec(
                "{$this->config['festival']['text2wave']} -F $frequency -o $fname.wav $fname.txt"
            );
        } else {
            touch("$fname.txt");
            touch("$fname.wav");
        ...
        foreach (glob(
            $this->config['phpagi']['tempdir'] . DIRECTORY_SEPARATOR . 'text2wav_*'
        ) as $file)
    ...
    public function swift($text, $escape_digits = '', $frequency = 8000, $voice = NULL)
    {
        ...
        if (!file_exists("$fname.wav")) {
            ...
            shell_exec(
                "{$this->config['cepstral']['swift']} -p audio/channels=1,audio/sampling-rate=$frequency $voice -o $fname.wav -f $fname.txt"
            );
        }
        ...
        $delete = time() - 2592000; // 1 month
        foreach (glob(
            $this->config['phpagi']['tempdir'] . DIRECTORY_SEPARATOR . 'swift_*'
        ) as $file)
            if (filemtime($file) < $delete)
                unlink($file);
    ...
    {
        $this->asm = new AGIAsteriskManager(NULL, $this->config);
        $this->asm->pagi = &$this;
    ...
    public function evaluate($command)
    {
        ...
        $ret['result'] = NULL;
        $ret['data'] = '';
        if ($ret['code'] != static::AGIRES_OK) // some sort of error
        {
            $ret['data'] = $str;
            $this->conlog(print_r($ret, true));
        } else // normal static::AGIRES_OK response
        {
     ...
     * @return void 
     */
    public function setSocketTimeoutInput($seconds)
    {
        stream_set_timeout($this->in, $seconds);
    }
     ...
     * @return void 
     */
    public function setSocketTimeoutOutput($seconds)
    {
        stream_set_timeout($this->out, $seconds);
    }
     ...
     */
    public function phpagiErrorHandler($level, $message, $file, $line, $context)
    {
